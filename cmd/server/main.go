...
func main() {
	...
	serverAddr := server.GetServerAddress(cfg.Server.Host, cfg.Server.Port)
	server := &http.Server{
		Addr:    serverAddr,
		Handler: h2c.NewHandler(mux, &http2.Server{}),
	}

	log.Printf("Starting chat server on %s (TLS: %v)", serverAddr, cfg.Server.TLSEnabled)

	errChan := make(chan error, 1)
	go func() {
		var err error
		if cfg.Server.TLSEnabled {
			// For production, you should provide proper TLS certificates
